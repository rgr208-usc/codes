...
AND TO_DATE(SUBSTRING(sale_derived_date FROM 1 FOR 8), 'YYYYMMDD')>'2015-01-01'
AND TO_DATE(SUBSTRING(sale_derived_date FROM 1 FOR 8), 'YYYYMMDD')<'2024-01-01'
;

select * from Eagle_Rock order by date

 SELECT non_missing, COUNT(owner_transfer_composite_transaction_id)  FROM Eagle_Rock GROUP BY non_missing
 SELECT primary_category_code, COUNT(owner_transfer_composite_transaction_id)  FROM Eagle_Rock GROUP BY primary_category_code
